...
def main() -> None:
    ...
    obs_space = dq.observation_space
    act_space = dq.action_space

    from stable_baselines3.common.policies import ActorCriticPolicy
    policy = CertiQSB3Policy(
        obs_space, act_space, nn.Tanh,
        network=network_t,
        mu=mu_t,
        alpha=0,
    ...
    # ── Prepare test data ───────────────────────────────────────────────────
    obs, _ = dq.reset()
    single_obs = torch.as_tensor(obs, dtype=torch.float32, device=device)
    batch_obs = single_obs.unsqueeze(0).expand(batch_size, -1).contiguous()
    actors_obs = single_obs.unsqueeze(0).expand(actors, -1).contiguous()

    # Single action and batch action
    action = policy(single_obs.unsqueeze(0))[0][0]
    dq_action = action.cpu().numpy()
    batch_action = action.unsqueeze(0).expand(batch_size, -1, -1).contiguous()
    ...
    print(f"Per-step workload: {actors} envs × (1 obs + 1 step) per timestep")
    print(f"{'─'*70}")
